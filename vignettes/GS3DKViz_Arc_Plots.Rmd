...
# mcols(regions(gs3dk_gint)) %>% dplyr::mutate(knockoff=ifelse(any(promoter.id %in% knockoff_genes | enhancer.id %in% knockoff_genes)))
mcols(regions(gs3dk_gint))$node.class
...
#interaction_track <- InteractionTrack(gs3dk_gint,name='HiC',chromosome = "chr1")
# displayPars(interaction_track)  <- list(col.interactions="black",
#     col.interactions.types=c('promoter-promoter'='red', 'promoter-enhancer'='blue'))
# mcols(gs3dk_gint) %>% tibble::as_tibble() %>% dplyr::select(gene_id,identified_by_knockoff_or_not) %>% dplyr::inner_join(x=.y=mcols(regions(gs3dk_gint)),by=c("gene_id"="promoter.id"))

##mcols(gs3dk_gint) %>% tibble::as_tibble() %>% dplyr::select(gene_id,identified_by_knockoff_or_not) %>% dplyr::inner_join(tidyr::unnest(tibble::as_tibble(mcols(regions(gs3dk_gint)))),by=c("gene_id" = "promoter.id"))
##gene_knock_df=mcols(gs3dk_gint) %>% tibble::as_tibble() %>%
##  dplyr::select(gene_id,identified_by_knockoff_or_not)
##mcols_regions_gs3dk=(tibble::as_tibble(mcols(regions(gs3dk_gint)))) %>% tidyr::unnest() %>% dplyr::mutate(interaction_id=1:length(regions(gs3dk_gint)))
# mcols(regions(gs3dk_gint)) %>% data.table::as.data.table() %>% dplyr::inner_join()
##mcols(regions(gs3dk_gint))$node.class
...
# displayPars(interaction_track)=list(col.interactions="black",col.interaction.types=c('promoter-promoter'='red', 'promoter-enhancer'='blue'))
displayPars(interaction_track)=list(col.interactions="black",col.interaction.types=c('knockoff_detected-knockoff_detected'='red', 'knockoff_detected-knockoff_removed'='blue','knockoff_removed-knockoff_removed'='blue'))
bounds=c(gs3dk_gint %>% as.data.frame() %>% janitor::clean_names() %>% dplyr::filter(trait=="FEV1/FVC") %>% dplyr::pull(start1) %>% min(),
gs3dk_gint %>% as.data.frame() %>% janitor::clean_names() %>% dplyr::filter(trait=="FEV1/FVC") %>% dplyr::pull(end1) %>% max(),
gs3dk_gint %>% as.data.frame() %>% janitor::clean_names() %>% dplyr::filter(trait=="FEV1/FVC") %>% dplyr::pull(start2) %>% min(),
gs3dk_gint %>% as.data.frame() %>% janitor::clean_names() %>% dplyr::filter(trait=="FEV1/FVC") %>% dplyr::pull(end2) %>% max())
...
```{r eval=F}
filename=gsub(":","_",gsub(" ","_",paste(date(),"AD-dense-details-extra.eps")))
